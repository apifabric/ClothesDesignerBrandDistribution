...
def declare_logic():
    ...
    # Calculate the total order amount for purchase orders of stores
    Rule.sum(derive=PurchaseOrder.total_amount, as_sum_of=Item.price, child_role_name='items', where=lambda row: row.collection_id == self.collection_id)

    # Ensure that events in a gallery do not exceed its capacity.
    Rule.constraint(validate=Gallery, as_condition=lambda gallery: gallery.capacity >= gallery.events.count(), error_msg="Events exceeding gallery capacity")
